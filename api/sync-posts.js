...
const WEBHOOK_SECRET = process.env.SYNC_WEBHOOK_SECRET || 'devinquire-secret-token-123';
const DASHBOARD_API_URL = process.env.DASHBOARD_API_URL || 'http://localhost:8000/api/public';
...
async function syncPostToMySQL(postData) {
  ...
  let categoryId;
  const [cats] = await db.query('SELECT id FROM blog_categories WHERE slug = ?', [categorySlug]);
  ...
  if (existing.length > 0) {
    ...
    await db.query(
      `UPDATE blog_posts SET 
       ...
       WHERE firestore_id = ?`,
      [title, slug, content, excerpt, categoryId, status, authorName, publishedAt, String(dashboardId)]
    );
  } else {
    // Insert new post
    const [postInsert] = await db.query(
      `INSERT INTO blog_posts 
       (firestore_id, title, slug, content, excerpt, category_id, 
        status, author_name, published_at, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
      [String(dashboardId), title, slug, content, excerpt, categoryId, status, authorName, publishedAt]
    );
  ...
  if (Array.isArray(tags) && tags.length > 0) {
    ...
    for (const tagName of tags) {
      ...
      // Find or create tag
      let tagId;
      const [exTags] = await db.query('SELECT id FROM blog_tags WHERE name = ?', [tagName]);
      if (exTags.length === 0) {
...
router.all('/', async (req, res) => {
  ...
  try {
    const listUrl = `${DASHBOARD_API_URL.replace(/\/$/, '')}/blog_list.php`;
    
    // Fetch posts list
    const listResponse = await fetch(listUrl);
    if (!listResponse.ok) {
      throw new Error(`Failed to fetch blog list. HTTP status ${listResponse.status}`);
    }
    ...
    for (const postOverview of postsList) {
      try {
        const detailUrl = `${DASHBOARD_API_URL.replace(/\/$/, '')}/blog_detail.php?id=${encodeURIComponent(postOverview.id)}`;
        
        // Fetch detailed post
        const detailResponse = await fetch(detailUrl);
        if (!detailResponse.ok) {
          console.warn(`Skipping post ID ${postOverview.id} details fetch failed: HTTP ${detailResponse.status}`);
          skipped++;
          continue;
        }
        ...
        console.error(`Error syncing post ID ${postOverview.id}:`, postError.message);
        skipped++;
      }
    }
