        ...
        <div className="px-3 sm:px-4 md:px-6 lg:px-8 transition-all duration-500">
          ...
          `}>
            ...
            <div className="w-[160px] sm:w-[180px] md:w-[200px] flex-shrink-0">
              ...
              >
                <div className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 rounded-lg sm:rounded-xl flex items-center justify-center transition-transform duration-200 group-hover:scale-110 bg-indigo-600 shadow-lg relative overflow-hidden mobile-logo">
                ...
                <span className="font-bold text-base sm:text-lg md:text-xl text-white flex items-center min-w-0 mobile-brand-text tracking-tight">
                ...
                <div className="flex items-center space-x-3 lg:space-x-6" style={{ pointerEvents: 'auto' }}>
                  {[
                    ...
                    <Link
                      key={path}
                      to={path}
                      className={`nav-item px-2 md:px-3 lg:px-4 py-2 font-medium relative group text-sm md:text-base transition-all duration-300 ease-out ${isScrolled ? 'text-slate-300 hover:text-white' : 'text-slate-400 hover:text-white'
                        }`}
                      style={{ pointerEvents: 'auto' }}
                      onClick={() => setIsMenuOpen(false)}
                    >
                      <span className="relative z-10 transition-all duration-300 ease-out">{label}</span>
                      <span className={`absolute bottom-0 left-0 h-0.5 bg-indigo-500 transition-all duration-500 ease-out transform origin-left ${isScrolled
                          ? 'w-0 group-hover:w-full opacity-100 scale-x-0 group-hover:scale-x-100'
                          : 'w-0 opacity-0 scale-x-0'
                        }`}></span>
                      <span className={`absolute inset-0 rounded-lg transition-all duration-300 ease-out ${isScrolled
                ...
                {currentUser ? (
                  <div className="relative">
                    <button
                      ...
                      className={`flex items-center space-x-1 lg:space-x-2 px-2 lg:px-4 py-2 rounded-full font-medium transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 ${isScrolled
                  ...
                  <div className="flex items-center space-x-2 md:space-x-3">
                    <Link
                      to="/contact"
                      className={`auth-button sign-in px-3 md:px-4 py-2 border-2 rounded-full font-medium transition-all duration-300 text-sm md:text-base whitespace-nowrap ${isScrolled
            ...
            <div className="md:hidden" style={{ pointerEvents: 'auto' }}>
              <button
                ...
                className={`w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-lg md:rounded-xl transition-all duration-300 ease-out focus:outline-none z-[70] relative transform-gpu mobile-menu-button ${isScrolled
              ...
              >
                <div className="w-5 h-5 md:w-6 md:h-6 relative flex items-center justify-center transform transition-all duration-300 ease-out mobile-hamburger">
                  <span className={`absolute h-0.5 w-5 md:w-6 transform transition-all duration-300 ease-out bg-gray-700 ${isMenuOpen ? 'rotate-45 translate-y-0' : '-translate-y-1.5'
                    }`} style={{ transformOrigin: 'center' }}></span>
                  <span className={`absolute h-0.5 w-5 md:w-6 transform transition-all duration-300 ease-out bg-gray-700 ${isMenuOpen ? 'opacity-0 scale-x-0' : 'opacity-100 scale-x-100'
                    }`} style={{ transformOrigin: 'center' }}></span>
                  <span className={`absolute h-0.5 w-5 md:w-6 transform transition-all duration-300 ease-out bg-gray-700 ${isMenuOpen ? '-rotate-45 translate-y-0' : 'translate-y-1.5'
                    }`} style={{ transformOrigin: 'center' }}></span>
                </div>
